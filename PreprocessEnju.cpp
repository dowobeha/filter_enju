...
void PreprocessEnju::readTree(const std::string& Parsed, std::string& outputLine, bool& parsedSuccess){
  ...
  */
  std::cerr << "\r"
	    << "success: " << this->successNum << ", other:" << this->fragmentalNum << std::flush;
}

bool PreprocessEnju::extractParsedSentence(const std::string& srcParsed, const std::string& srcFileName, 
  ...
  if (! ifsSrcParsed) {
    std::cerr << "Failed to open parsed source file:\t" << srcParsed << std::endl;
  }
  if (! ifsTgt) {
    std::cerr << "Failed to open target file:\t" << tgt << std::endl;
  }
  if (! ifsSrcParsed || ! ifsTgt) {
    return false;
  }
  ...
  for (std::string srcLine, tgtLine; std::getline(ifsSrcParsed, srcLine) && std::getline(ifsTgt, tgtLine);){
    ...
    std::cerr << ", Progress: " << numLineLess << "/" << ++numLine << " lines" << std::flush;
  }
